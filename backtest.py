...
def simulate_trade(df, entry_idx, direction, entry, sl_price, tp1_price, tp2_price, tp3_price):
    """Simulate a trade with partial TP exits."""
    tp_remaining = 1.0
    realized_pnl = 0.0
    exit_type = None
    bars_held = 0
    ...
    for j in range(entry_idx + 1, min(entry_idx + 120, len(df))):
        ...
        if direction == "LONG":
            ...
            # TP3 (10%)
            if 0.01 <= tp_remaining <= 0.11 and high >= tp3_price:
            ...
            # TP3 (10%)
            if 0.01 <= tp_remaining <= 0.11 and low <= tp3_price:
                tp_size = tp_remaining
    ...
    if exit_type is None and tp_remaining > 0:
        exit_type = "TIMEOUT"
...
def send_telegram_report(total_trades, wins, losses, win_rate,
    ...
    text = (
        ...
        f"  <i>TP1: 1.0% (50%) | TP2: 3.0% (40%) | TP3: 6.0% (10%)</i>"
    )
