...
def build_report(all_trades):
    ...
    is_win_list = (df_sorted['pnl'] > 0).tolist()
    max_win_streak = 0
    max_loss_streak = 0
    current_streak = 0
    current_type = None
    for val in is_win_list:
        if val == current_type:
            current_streak += 1
        else:
            current_type = val
            current_streak = 1
        if current_type is True and current_streak > max_win_streak:
            max_win_streak = current_streak
        if current_type is False and current_streak > max_loss_streak:
