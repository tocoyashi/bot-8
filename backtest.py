...
def simulate_trade(df, entry_idx, direction, entry, sl_price, tp1_price, tp2_price, tp3_price):
    """Simulate a trade with partial TP exits. SL moves to entry (BE) after TP1."""
    tp_remaining = 1.0
    realized_pnl = 0.0
    exit_type = None
    bars_held = 0
    ...
    be_sl_price = sl_price  # starts at original SL
    ...
    for j in range(entry_idx + 1, min(entry_idx + 120, len(df))):
        ...
        if direction == "LONG":
            # Check SL (may be original or breakeven)
            if low <= be_sl_price:
                pnl = (be_sl_price - entry) / entry * 100 * tp_remaining
                realized_pnl += pnl
                exit_type = "BE" if tp1_hit and be_sl_price >= entry else "SL"
                break
            ...
            if not tp1_hit and high >= tp1_price:
                ...
                tp1_hit = True
                be_sl_price = entry  # Move SL to entry (breakeven)
            ...
            # Check SL (may be original or breakeven)
            if high >= be_sl_price:
                pnl = (entry - be_sl_price) / entry * 100 * tp_remaining
                realized_pnl += pnl
                exit_type = "BE" if tp1_hit and be_sl_price <= entry else "SL"
                break
