...
def simulate_trade(df, entry_idx, direction, entry, sl_price, tp1_price, tp2_price, tp3_price):
    """Simulate a trade with partial TP exits and trailing SL to breakeven."""
    tp_remaining = 1.0
    realized_pnl = 0.0
    exit_type = None
    bars_held = 0
    sl_moved = False  # Track if SL moved to breakeven after TP1
    ...
    for j in range(entry_idx + 1, min(entry_idx + 120, len(df))):
        ...
        # Current active SL
        active_sl = be_sl_price if sl_moved else sl_price

        if direction == "LONG":
            # Check TP1 first (closer to entry than SL)
            if tp_remaining > 0.49 and high >= tp1_price:
                tp_size = min(0.50, tp_remaining)
                pnl = (tp1_price - entry) / entry * 100 * tp_size
                realized_pnl += pnl
                tp_remaining -= tp_size
                sl_moved = True  # Move SL to breakeven
                # If also hit SL (breakeven) on same bar, close remaining at 0
                if low <= be_sl_price and tp_remaining > 0.01:
                    tp_remaining = 0
                    exit_type = "TP1+BE"
                    break
                # If also hit TP2 on same bar
                if tp_remaining > 0.01 and high >= tp2_price:
                    tp_size = min(0.25, tp_remaining)
            ...
            if low <= active_sl:
                if sl_moved:
                    # Breakeven SL - 0 loss on remaining
                    exit_type = "BE" if tp_remaining > 0.01 else exit_type
                else:
                    pnl = (active_sl - entry) / entry * 100 * tp_remaining
                    realized_pnl += pnl
                    exit_type = "SL"
                break

            # TP2 (25%)
            if 0.01 < tp_remaining <= 0.50 and high >= tp2_price:
                tp_size = min(0.25, tp_remaining)
                pnl = (tp2_price - entry) / entry * 100 * tp_size
                realized_pnl += pnl
                tp_remaining -= tp_size
                if tp_remaining < 0.01:
                    exit_type = "TP_ALL" if sl_moved else "TP2"
                    break
                # Check TP3 same bar
                if high >= tp3_price:
                    pnl = (tp3_price - entry) / entry * 100 * tp_remaining
            ...
            # TP3 (25%)
            if 0.01 <= tp_remaining <= 0.26 and high >= tp3_price:
                tp_size = tp_remaining
                pnl = (tp3_price - entry) / entry * 100 * tp_size
                realized_pnl += pnl
                tp_remaining = 0
                exit_type = "TP_ALL"
                break
            ...
            if tp_remaining > 0.49 and low <= tp1_price:
                tp_size = min(0.50, tp_remaining)
                pnl = (entry - tp1_price) / entry * 100 * tp_size
                realized_pnl += pnl
                tp_remaining -= tp_size
                sl_moved = True  # Move SL to breakeven
                # If also hit SL (breakeven) on same bar
                if high >= be_sl_price and tp_remaining > 0.01:
                    tp_remaining = 0
                    exit_type = "TP1+BE"
                    break
                # If also hit TP2 on same bar
                if tp_remaining > 0.01 and low <= tp2_price:
                    tp_size = min(0.25, tp_remaining)
            ...
            if high >= active_sl:
                if sl_moved:
                    ...
                    pnl = (entry - active_sl) / entry * 100 * tp_remaining
                    realized_pnl += pnl
                    exit_type = "SL"
                break

            # TP2 (25%)
            if 0.01 < tp_remaining <= 0.50 and low <= tp2_price:
                tp_size = min(0.25, tp_remaining)
                pnl = (entry - tp2_price) / entry * 100 * tp_size
                realized_pnl += pnl
                tp_remaining -= tp_size
            ...
            # TP3 (25%)
            if 0.01 <= tp_remaining <= 0.26 and low <= tp3_price:
                tp_size = tp_remaining
                pnl = (entry - tp3_price) / entry * 100 * tp_size
                realized_pnl += pnl
                tp_remaining = 0
                exit_type = "TP_ALL"
                break
...
def send_telegram_report(total_trades, wins, losses, win_rate,
    ...
    text = (
        ...
        f"  <i>TP1: 1.0% (50%) | TP2: 3.0% (25%) | TP3: 6.0% (25%) | SL->BE after TP1</i>"
    )
